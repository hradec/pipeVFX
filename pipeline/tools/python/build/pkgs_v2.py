...
class all: # noqa
    ...
    def __init__(self,ARGUMENTS):
        ...
        gcc = build.gccBuild(
                ...
                download=[(
                   ...
                   { build.override.src: 'gcc.spec' } #, self.gcc_4_1_2: None }
                    ...
                    # CY2021-CY2022 GCC 9.3.1 source code.
                   ...
                   { build.override.src: 'gcc.spec' } #, self.gcc_4_1_2: None }
                # ),(
                #     # CY2023 GCC 11.2.1 source code.
                #    'https://cbs.centos.org/kojifiles/packages/devtoolset-11-gcc/11.2.1/1.1.el7/src/devtoolset-11-gcc-11.2.1-1.1.el7.src.rpm',
                #    'gcc-11.2.1.rpm.tar.gz',
                #    '11.2.1',
                #    '3470cd70f1547d8abc2f2330e30ebfea',
                #    { build.override.src: 'gcc.spec' } #, self.gcc_4_1_2: None }
                )],
        ...
        self.doxygen = doxigen
        build.globalDependency(self.doxygen)

        self.partio = build.cmake(
            ARGUMENTS,
            'partio',
            download=[(
                'https://github.com/wdas/partio/archive/refs/tags/v1.14.6.tar.gz',
                'partio-1.14.6.tar.gz',
                '1.14.6',
                '4c2a2bdb79560d381191f5f94464f7cb',
                { self.gcc: '6.3.1' }
            )],
            depend = [self.doxigen],
        )
        # build.globalDependency(self.partio)
        ...
        for boost_version in ['1.51.0']+self.boost_versions_for_gaffer:
            ...
            ilmbase = build.configure(
                ...
                download=[(
                    ...
                    { self.gcc : self.boost[boost_version]['gcc'].version, python: '2.7.16', boost: boost_version },
                    { 'python' : ('2.0.0','3.0.0'), "boost" : ('0.0.0', '1.51.0')} # not implemented yet
                ),(
        ...
        for boost_version in ['1.51.0']+self.boost_versions_for_gaffer:
            ...
            download=[(
                ...
                { self.gcc : self.boost[boost_version]['gcc'].version, python: '2.7.16', self.ilmbase[sufix]: '2.2.0', boost: boost_version },
                { 'python' : ('2.0.0','3.0.0'), "boost" : ('0.0.0', '1.60.0')}
            # ),(
            #     # CY2019
        ...
        for boost_version in ['1.51.0']+self.boost_versions_for_gaffer:
            ...
            pyilmbase = build.configure(
                ...
                download=[(
                    ...
                    { self.gcc : self.boost[boost_version]['gcc'].version, self.python: '2.7.16', self.ilmbase[sufix]: '2.2.0', self.openexr[sufix]: '2.2.0', self.boost: boost_version },
                    { 'python' : ('2.0.0','3.0.0'), "boost" : ('0.0.0', '1.61.0')}
                # ),(
                ...
                cmd = [
                    ...
                    'cd $SOURCE_FOLDER && make LDFLAGS="$LDFLAGS" -j $DCORES install',
                    'find ./PyImath/ -name "imathmodule.so" -exec cp -rfv {} $INSTALL_FOLDER/lib/python$PYTHON_VERSION_MAJOR/site-packages/ \; ',
        ...
        self.yaml_cpp = build.cmake(
            ...
            download=[(
                ...
                'yaml-cpp-yaml-cpp-0.7.0.tar.gz',
                '0.7.0',
        ...
        glfw = build.glfw(
            ...
            depend=[python, glew, self.doxygen],
            flags = [
        ...
        # =============================================================================================================================================
        # a fix for building with gcc and llvm toguether
        # =============================================================================================================================================
        ...
        for boost_version in ['1.51.0']+self.boost_versions_for_gaffer:
            ...
            #     ' -DCHAR_BIT=8 -DSCHAR_MIN=-127 -DSCHAR_MAX=127 -DUCHAR_MAX=255  -DCHAR_MIN=0 -DCHAR_MAX=UCHAR_MAX -DSHRT_MIN=-32767 -DSHRT_MAX=32767 ',
            #     ' -DUSHRT_MAX=65535 -DINT_MIN=-32767 -DINT_MAX=32767 -DUINT_MAX=65535  -DLONG_MIN=-2147483647 -DLONG_MAX=2147483647 -DULONG_MAX=4294967295 ',
            #     ' -DLLONG_MIN=-9223372036854775807 -DLLONG_MAX=9223372036854775807  -DULLONG_MAX=18446744073709551615 ',
            # ])

            # openvdb needs to build with gcc and llvm at the same time, so we
        ...
        # =============================================================================================================================================
        # BUILD PACKAGES THAT DEPEND ON OPENEXR
        # =============================================================================================================================================
        self.osl = {}
        self.materialx = {}
        self.alembic = {}
        ...
        # =============================================================================================================================================
        # EMBREE - Just download the binaries (gaffercycles)
        # =============================================================================================================================================
        self.embree_bin = build.download(
            ...
            download=[(
                ...
                'embree-3.13.4.x86_64.linux.tar.gz',
                '3.13.4',
                'c361c86310ad81ad2f3235a54801a692',
                { build.override.src: 'embree-vars.csh' }
            )],
            environ = environ,
        )
        ...
        for bv in self.boost_versions_for_gaffer:
            ...
            # github build point so we can split the build in multiple matrix jobs in github actions
            # ============================================================================================================================================
            for v in self.embree[bsufix].keys():
            ...
            osl_flags = [
                ...
                '-D STOP_ON_WARNING=0',
                '-D Qt5_ROOT=$QT_TARGET_FOLDER',
                '-D Qt5_HOME=$QT_TARGET_FOLDER',
                '-D QT5_HOME=$QT_TARGET_FOLDER',
            ]
            osl = build.cmake(
                ARGUMENTS,
                ...
                depend=[self.icu, self.cmake, self.pugixml, self.freetype,
                        ...
                        self.partio], #, self.optix, self.cuda],
                cmd = [
        ...
        for bv in ['1.51.0']+self.boost_versions_for_gaffer:
            ...
            if bv == '1.51.0':
                download += [(
                    ...
                    {self.gcc: self.boost['1.51.0']['gcc'].version, self.boost: '1.51.0', self.hdf5: '1.8.11',
        ...
        for bv in self.boost_versions_for_gaffer:
            ...
            opensubdiv = build.cmake(
                ...
                depend=[
                    self.hdf5, self.glfw,
                    self.glew, self.clew[bsufix],
                    # self.cuda, self.doxygen,
                ],
                flags=[
            ...
            seexpr = build.cmake(
                ...
                depend = [python, qt],
                flags = [
                    '-D CMAKE_MODULE_PATH=$CMAKE_PREFIX_PATH',
                    '-D Qt5_DIR=$QT_TARGET_FOLDER/lib/cmake/Qt5/'
                ]+build.cmake.flags,
                cmd=[
                ...
                for MONOLITHIC in [1,0]:
                    ...
                    usd = build.cmake(
                        ...
                        cmd = [
                            ...
                            "cmake --build --target install --parallel $CORES -S ..",
        ...
        self.cgru = build.configure(
            ...
            # ),(
            #     'https://github.com/hradec/cgru/archive/refs/tags/3.3.1-pipevfx.tar.gz',
            #     'cgru-3.3.1-pipevfx.tar.gz',
            #     '3.3.1',
            #     '9feafcfa865ac3cdfe61b77d0cc901f3',
            #     {   self.gcc : '6.3.1',
            #         self.boost: cgru_boost_version,
        ...
        #             ]:
        #     for v in pkg.keys():
        #         build.github_phase_one_version(ARGUMENTS, {pkg : v})
        ...
        #     #     hdf5: '1.8.11', cmake: '3.18.2', tbb: '2020_U3',  qt: '5.6.1',
        ...
        #         { gcc: '6.3.1', opensubdiv: '3.4.0', alembic: '1.7.11',
        #         hdf5: '1.8.11', cmake: '3.18.2', tbb: '2020_U3',  qt: '4.8.7',
