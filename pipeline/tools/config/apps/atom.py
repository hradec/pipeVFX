...
class atom(baseApp):
    ...
    def macfix(self, macfixData):
        ...
        if os.path.exists(self.path("usr")):
            # starting on version 1.60, atom folder is just the uncompressed .deb
            # file. So we choose to keep the folder structure found in the .deb
            # which starts at /usr, /opt, etc.
            macfixData['subpath'] = 'usr/'
