...
class shave(ParameterisedProcedural) :
    ...
    def __init__(self) :
        ...
        m_targetMeshParameter = MeshPrimitiveParameter(
	        "target",
        ...
        maxDiff  = FloatParameter("maxDiffuseDepth", "maximun depth a ray will go to get indirect diffuse",1)
    ...
    def doBound(self, args) :
        ...
        if f:
            ...
            #     id += [count] * c
            #     count += 1
            # self.__id = PrimitiveVariable( PrimitiveVariable.Interpolation.Vertex, IntVectorData( id ) )
    ...
    def doRender(self, renderer, args):
        ...
        with AttributeBlock( renderer ) :
            if renderer.typeName()!="IECoreRI::Renderer" :
                ...
                ce = IECore.CurvesPrimitiveEvaluator.create(geo)
                resultC = ce.createResult()

                hide = [0.0] * geo.numCurves()
                for n in range(len(mesh['P'].data)):
                    ce.closestPoint(mesh['P'].data[n], resultC)
                    dot = (resultC.point()-mesh['P'].data[n]).dot( mesh['N'].data[n] )
                    if dot<0.0:
                        hide[ resultC.curveIndex() ] = 0.0
                        print resultC.curveIndex()

                geo['hide'] = PrimitiveVariable( PrimitiveVariable.Interpolation.Uniform, FloatVectorData( hide ) )
                ...
                renderer.setAttribute( "ri:dice:hair", FloatData( args['diceAsHair'].value ) )
                renderer.setAttribute( "ri:dice:roundcurve", FloatData( args['curvesAsTubes'].value ) )
                renderer.setAttribute( "ri:visibility:camera", FloatData( args['visibilityCamera'].value ) )
                renderer.setAttribute( "ri:visibility:indirect", FloatData( args['visibilityIndirect'].value ) )
                renderer.setAttribute( "ri:visibility:transmission", FloatData( args['visibilityTransmission'].value ) )
                renderer.setAttribute( "ri:trace:maxdiffusedepth", FloatData( args['maxDiffuseDepth'].value ) )
                renderer.setAttribute( "ri:trace:maxspeculardepth", FloatData( args['maxSpecularDepth'].value ) )
                renderer.setAttribute( "ri:shade:transmissionhitmode", 'shader' )
                renderer.setAttribute( "ri:grouping:membership", StringData( args['traceset'].value ) )
