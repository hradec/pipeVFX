...
if [ "$STUDIO" != "atomo" ] ; then
    mkdir -p /$STUDIO/pipeline/build/.download/
    mount -o bind /atomo/pipeline/build/.download/ /$STUDIO/pipeline/build/.download/
...
if [ "$RUN_SHELL" == "1" ] ; then
    ...
    sudo groupdel users > /dev/null 2>&1
    groupadd -g $_GID $_GROUP
    echo "$(cat /etc/default/useradd)" | sed "s/100/$_GID/" > /etc/default/useradd
