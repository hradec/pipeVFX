...
def main(resx, resy, output_path, scene_file):
    cmds=[]
    imgs=[]
...
if len(sys.argv)<2:
    ...
    kickCropAF - submit one frame to render in multiple machines on the farm (arnold ass file)
    
    
        example: kickCropAF  <path to ass file> <resolution X> <resolution Y>  <number of tiles>
    ...
    cmds, files = main(res_x, res_y, path, path_f )
