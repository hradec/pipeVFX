...
if ap.isValid():
    ...
    # just a path - assume images are in this path
    imagePath = assetPath
...
webplayerPath = "%s/.webplayer/" % imagePath
webplayerMontagePath = "%s/.webplayer_montage/" % imagePath

montageImages = {}
images = glob.glob( "%s/*" % imagePath )
images.sort()
for image in images:
    frame = image.split('.')[-2]
    print frame
    if frame not in montageImages:
        montageImages[frame] = []
    cmd = pipe.frame.webplayer( image, webplayerPath, montageImages[frame] )
